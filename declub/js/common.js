...
if(document.querySelector('.calculator')) {
  ...
  const calcRadioLabels = document.querySelectorAll('.form-calc__val');
  const calcMetersInput = document.querySelector('.metrs-value');
  const calcWorkRadios = document.querySelectorAll('.custom-radio__input');
  const yourHoursText = document.querySelector('.js-your-hours');
  const ourHoursText = document.querySelector('.js-our-hours');
  ...
  function setCalcData() {
    ...
    // Set object type
    let calcRadios = document.querySelectorAll('.form-calc__val input');
    ...
    // set object area
    calcData.area = parseInt(calcMetersInput.value)
  
    // set design
    let withDesign = document.querySelector('.custom-radio__input[value="design"]').checked;
...
$(document).ready(function () {
  ...
  function resizeHeaderSlider() {
    $('.page-header__slider').css('margin-top', `-${$('.page-header .container').height() + $('.page-header__top').height()}px`);
    $('.page-header__slider').css('height', `${$('.page-header .container').height() + $('.page-header__top').height()}px`);
  } resizeHeaderSlider();
  

  function changeTabsText() {
    $(window).width() >= 320 && $(window).width() <= 1199 
    ? $('.tab__value').text('м') 
    : $('.tab__value').text('метров')
    $(window).width() >= 320 && $(window).width() <= 1199 
    ? $('.price-text').text('узнайте стоимость ремонта для своего объекта') 
    : $('.price-text').text('или узнайте стоимость ремонта для своего объекта')
  } changeTabsText();
  ...
  $(".video-item__bg").each(function () {
    ...
    $(this).attr(`style`, `background-image: url(https://img.youtube.com/vi/${this.id}/sddefault.jpg)`);
  });
